...
test_that("aprlb works with GKB dataset", {
  data("GKB")
  res <- aprlb(y = "voteddem_all", z = "post", data = GKB)
...
test_that("aprub works with GKB dataset", {
  data("GKB")
  res <- aprub(y = "voteddem_all", t = "readsome", z = "post", data = GKB)
...
test_that("lpr4ytz works with GKB dataset", {
  data("GKB")
  res <- lpr4ytz(y = "voteddem_all", t = "readsome", z = "post", data = GKB)
...
test_that("calc4persuasio works with GKB summary statistics", {
  ...
  res <- calc4persuasio(y1 = y1, y0 = y0, e1 = e1, e0 = e0)
  ...
  res_partial <- calc4persuasio(y1 = y1, y0 = y0)
...
test_that("persuasio4ytz returns structured result with GKB", {
  data("GKB")
  res <- persuasio4ytz(y = "voteddem_all", t = "readsome", z = "post", data = GKB)
  expect_true(is.list(res))
...
test_that("persuasio4yz returns scalar bound with GKB", {
  data("GKB")
  res <- persuasio4yz(y = "voteddem_all", z = "post", data = GKB)
  expect_true(is.numeric(res$lb_coef))
  expect_true(res$lb_coef >= 0 && res$lb_coef <= 1)
})
...
test_that("persuasio4ytz2lpr returns lpr estimate with GKB", {
  data("GKB")
  res <- persuasio4ytz2lpr(y = "voteddem_all", t = "readsome", z = "post", data = GKB)
  expect_true(is.numeric(res$lpr))
  expect_true(res$lpr >= 0 && res$lpr <= 1)
})
...
test_that("persuasio wrapper routes to apr correctly with GKB", {
  data("GKB")
  res <- persuasio(est = "apr", y = "voteddem_all", t = "readsome", z = "post", data = GKB)
  expect_true(!is.null(res))
  expect_true(is.list(res))
  expect_true(!is.null(res$lb_coef))
})

test_that("persuasio wrapper routes to yz correctly with GKB", {
  data("GKB")
  res <- persuasio(
    est = "yz", y = "voteddem_all", z = "post", x = "MZwave2",
    data = GKB, method = "bootstrap", nboot = 5
  )
  expect_s3_class(res, "persuasio4yz")
  expect_equal(res$outcome, "voteddem_all")
  expect_equal(res$instrument, "post")
  expect_equal(res$covariates, "MZwave2")
})
